...
cmake -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DCMAKE_INSTALL_PREFIX=../../install_root -DMAGNUM_TARGET_EGL=ON -DMAGNUM_WITH_WINDOWLESSEGLAPPLICATION=ON \
  -DEGL_INCLUDE_DIR="${EGL_INCLUDE_DIR}" \
  -DEGL_LIBRARY="${EGL_LIBRARY}" \
  -DMAGNUM_WITH_GLXAPPLICATION=OFF \
  -DMAGNUM_WITH_GLXCONTEXT=OFF \
  -DMAGNUM_WITH_SDL2APPLICATION=OFF \
  -DMAGNUM_WITH_XEGLAPPLICATION=OFF \
  -DMAGNUM_WITH_OPENGLTESTER=OFF \
